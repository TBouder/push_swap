...
/*   Updated: 2016/04/13 13:59:54 by tbouder          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_calls_push(t_swap *a, t_swap *b, t_flag *flg)
{
	...
	if (ft_z(a) < ft_y(a))
	{
		...
		// flg->prev == 0 && ft_z(a) > A ? ft_rrr_a(a, b, flg) : 0;
		flg->prev < 2 && ft_z(a) > A ?ft_putnbr(flg->prev), ft_rrr_a(a, b, flg) : 0;
		// flg->prev == 1 && ft_z(a) > A && ft_y(a) > ft_z(a) ? ft_rr_a(a, b, flg) : 0;
		flg->prev == 2 && ft_z(a) > A ?ft_putnbr(flg->prev), ft_rr_a(a, b, flg) : 0;
...
void		ft_calls_rev_rot(t_swap *a, t_swap *b, t_flag *flg)
{
	...
	if (ft_z(a) < ft_y(a))
	{
		...
		ft_z(a) > A && !BNULL && ft_z(b) > ft_y(b) && ft_z(b) < B && ft_order(a) ? ft_rr_ab(a, b, flg) : 0;
		ft_z(a) > A && !ft_order(a) ? ft_rrr_a(a, b, flg) : 0;
		ft_z(a) > A && ft_order(a) ? ft_rr_a(a, b, flg) : 0; //AKI
		ft_z(a) < A && !BNULL && ft_z(b) < ft_y(b) ? ft_rs_b(a, b, flg) : 0;
	}
	ft_z(b) > ft_y(b) && ft_z(b) < B ? ft_rrr_b(a, b, flg) : 0;
}
