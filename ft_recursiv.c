...
void		ft_calls_swap(t_swap *a, t_swap *b, t_flag *flg)
{
	if (ft_z(a) < ft_y(a))
	{
		ft_z(a) > a->stack && ft_z(b) > ft_y(b) && ft_z(b) < b->stack
			&& ft_swaprev(a) ? ft_rrr_ab(a, b, flg) : 0;
		ft_z(a) > a->stack && ft_z(b) > ft_y(b) && ft_z(b) < b->stack
			&& !ft_swaprev(a) ? ft_rr_ab(a, b, flg) : 0; // ft_rr_ab(a, b, flg);
		ft_z(a) > a->stack && ft_z(b) > a->stack ? ft_rpb_a(a, b, flg) : 0;
		ft_z(a) > a->stack ? ft_rr_a(a, b, flg) : 0; // !ft_swaprev(a) ? ft_rr_a(a, b, flg) : ft_rrr_a(a, b, flg);
		ft_z(a) < a->stack ? ft_rpa_b(a, b, flg) : 0;
	}
	ft_z(b) > ft_y(b) && ft_z(b) < b->stack ? ft_rr_b(a, b, flg) : 0;
}
